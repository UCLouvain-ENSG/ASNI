...
void asni_fill(struct rte_mbuf *pkt, struct descriptor *desc) {

#ifdef ASNI_DESC_SIZE
    desc->size = pkt->data_len;
#endif
#if defined(ASNI_DESC_MAC_SRC) || defined(ASNI_DESC_MAC_DST) ||      \
    defined(ASNI_DESC_ETH_TYPE)
...
#endif
#if defined(ASNI_DESC_IP_SRC) || defined(ASNI_DESC_IP_DST) ||        \
    defined(ASNI_IP_PROTO)
...
#endif
#if defined(ASNI_DESC_PORT_SRC) || defined(ASNI_DESC_PORT_DST)
    if (RTE_ETH_IS_IPV4_HDR(pkt->packet_type)) {
