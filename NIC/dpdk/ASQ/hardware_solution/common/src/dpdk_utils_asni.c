 ...
 */
static void update_flow_action_rss(const uint16_t *queue_list,
                                   uint8_t nb_queues,
...
dpdk_sft_init(const struct application_dpdk_config *app_dpdk_config) {
    ...
    struct rte_sft_conf sft_config = {
        ...
        .tcp_ct_enable = app_dpdk_config->sft_config
                             .enable_ct, /* Enable TCP Connection Tracking */
        .ipfrag_enable = app_dpdk_config->sft_config
                             .enable_frag, /* Enable fragmented packets */
    ...
    update_flow_action_rss(queue_list, nb_queues, &rss_conf,
                           RTE_ETH_HASH_FUNCTION_DEFAULT, level, &action_rss);
    ...
    result = create_rules_sft_offload(app_dpdk_config, &action_rss,
                                      &action_rss_hairpin);
    if (result != DOCA_SUCCESS) {
    ...
    for (port_id = 0, n = 0; port_id < RTE_MAX_ETHPORTS; port_id++) {
        ...
        if (result != 0)
            DOCA_LOG_ERR("Failed getting device (port %u) info, error=%s",
                         port_id, strerror(-result));

        result = rte_dev_dma_unmap(dev_info.device, ext_mem->buf_ptr,
...
int rx_queue_setup(uint16_t port_id, uint16_t rx_queue_id, uint16_t nb_rx_desc,
    ...
    rx_pkt_seg_lengths[0] = 50;
    rx_pkt_nb_segs = 2;
    rx_pkt_seg_offsets[0] = 0;
    rx_pkt_nb_offs = 1;
    ...
    for (i = 0; i < rx_pkt_nb_segs; i++) {
        ...
        rx_seg->length = rx_pkt_seg_lengths[i] ? rx_pkt_seg_lengths[i]
                                               : mbuf_data_size[mp_n];
...
static doca_error_t port_init(struct rte_mempool *mbuf_pool, uint8_t port,
    ...
    if (enable_rx_split) {
        ...
        for (q = 0; q < rx_rings; q++) {
            ret = rx_queue_setup(port, q, RX_RING_SIZE,
    ...
    if (nb_hairpin_queues && app_config->port_config.self_hairpin &&
...
static doca_error_t dpdk_ports_init(struct application_dpdk_config *app_config,
                                    bool enable_split, int starting_port) {
    doca_error_t result;
    int ret;
    uint16_t port_id;
    uint16_t n;
    ...
    printf("inside dpdk port init dpdk_utils_asni.c\n");

    /* Initialize mbufs mempool */
#ifdef GPU_SUPPORT
    if (app_config->pipe.gpu_support)
        result = allocate_mempool_gpu(total_nb_mbufs, &app_config->pipe,
                                      &app_config->mbuf_pool);
    else
        result = allocate_mempool(total_nb_mbufs, &app_config->mbuf_pool);
#else
    /*FIXME x8*/
    printf("ALLOCATING MEMPOOL\n");
    result = allocate_mempool(total_nb_mbufs, &app_config->mbuf_pool);
#endif
    if (result != DOCA_SUCCESS)
        return result;
    ...
    for (port_id = starting_port, n = 0; port_id < RTE_MAX_ETHPORTS;
         port_id++) {
        if (!rte_eth_dev_is_valid_port(port_id))
            continue;
        printf("Initializing doca port %d\n", port_id);
        result =
            port_init(app_config->mbuf_pool, port_id, app_config, enable_split);
        if (result != DOCA_SUCCESS) {
...
dpdk_queues_and_ports_init(struct application_dpdk_config *app_dpdk_config) {
    ...
    if (app_dpdk_config->port_config.nb_ports > 0) {
        result = dpdk_ports_init(app_dpdk_config, false, 0);
        if (result != DOCA_SUCCESS) {
...
doca_error_t dpdk_queues_and_ports_init_split(
    struct application_dpdk_config *app_dpdk_config, bool enable_rx_split,
    int starting_port) {
    doca_error_t result;
    int ret = 0;
    printf("Initializing queues and ports\n");
    ...
    if (app_dpdk_config->port_config.nb_ports > 0) {
        result =
            dpdk_ports_init(app_dpdk_config, enable_rx_split, starting_port);
        if (result != DOCA_SUCCESS) {
            DOCA_LOG_ERR("Ports allocation failed");
            goto gpu_cleanup;
        }
