...
#define MSG_HOST_BUFF_BSIZE 2048
...
int main(int argc, char **argv) {
    ...
    app_cfg.nb_workers = 1;
    result = register_l2_reflector_params();
    if (result != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Failed to register application params: %s",
                     doca_error_get_descr(result));
        doca_argp_destroy();
        return EXIT_FAILURE;
    }
    ...
    DOCA_LOG_INFO("Running with %d workers\n", app_cfg.nb_workers);
    ...
    if (flexio_msg_stream_create(app_cfg.flexio_process, &stream_fattr, stdout,
                                 NULL, &app_cfg.stream)) {
        DOCA_LOG_ERR("Failed to create stream for messaging environment\n");
        goto device_cleanup;
    }
    ...
    for (int i = 0; i < app_cfg.nb_workers; i++) {
        ret = flexio_process_call(app_cfg.flexio_process,
                                  &l2_reflector_device_init, &rpc_ret_val,
    ...
    for (int i = 0; i < app_cfg.nb_workers; i++) {
        ret = flexio_event_handler_run(app_cfg.event_handler[i], (uint64_t)i);
        if (ret != FLEXIO_STATUS_SUCCESS) {
    ...
    while (!force_quit)
        sleep(1);
    // Avoids race conditions somehow for ressource cleaning but FIXME plz
    sleep(3);
...
device_cleanup:
    ...
    if (app_cfg.flexio_process && flexio_msg_stream_destroy(app_cfg.stream)) {
        DOCA_LOG_ERR("Failed to destroy FlexIO stream");
    }
ibv_device_cleanup:
