...
doca_error_t l2_reflector_setup_device(struct l2_reflector_config *app_cfg) {
    ...
    result = flexio_process_create(app_cfg->ibv_ctx, l2_reflector_device, NULL,
    ...
    for (int i = 0; i < app_cfg->nb_workers; i++) {
        ...
        result = flexio_event_handler_create(app_cfg->flexio_process,
                                             &event_handler_attr,
                                             &app_cfg->event_handler[i]);
        if (result != FLEXIO_STATUS_SUCCESS) {
 ...
 */
static doca_error_t allocate_dbr(struct flexio_process *process,
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t allocate_cq_memory(struct flexio_process *process,
                                       int log_depth,
                                       struct app_transfer_cq *app_cq) {
    ...
    ret = flexio_copy_from_host(process, cq_ring_src, ring_bsize,
                                &app_cq->cq_ring_daddr);
    free(cq_ring_src);
    if (ret) {
        DOCA_LOG_ERR("Failed to allocate CQ ring");
        return DOCA_ERROR_DRIVER;
...
static doca_error_t create_dpa_mkey(struct flexio_process *process,
                                    struct ibv_pd *pd, flexio_uintptr_t daddr,
                                    int log_bsize, int access,
...
static doca_error_t allocate_sq(struct l2_reflector_config *app_cfg) {
    ...
    for (int i = 0; i < app_cfg->nb_workers; i++) {
        ...
        result =
            allocate_cq_memory(app_cfg->flexio_process, L2_LOG_CQ_RING_DEPTH,
                               &app_cfg->sq_cq_transf[i]);
        if (result != DOCA_SUCCESS)
            return result;

        sqcq_attr.cq_dbr_daddr = app_cfg->sq_cq_transf[i].cq_dbr_daddr;
        sqcq_attr.cq_ring_qmem.daddr = app_cfg->sq_cq_transf[i].cq_ring_daddr;

        /* Create SQ's CQ */
        ret = flexio_cq_create(app_cfg->flexio_process, app_cfg->ibv_ctx,
                               &sqcq_attr, &app_cfg->flexio_sq_cq_ptr[i]);
        if (ret != FLEXIO_STATUS_SUCCESS) {
            DOCA_LOG_ERR("Failed to create FlexIO SQ's CQ");
            return DOCA_ERROR_DRIVER;
        }

        cq_num = flexio_cq_get_cq_num(app_cfg->flexio_sq_cq_ptr[i]);
        app_cfg->sq_cq_transf[i].cq_num = cq_num;
        app_cfg->sq_cq_transf[i].log_cq_depth = L2_LOG_CQ_RING_DEPTH;

        /* Allocate memory for SQ */
        log_sqd_bsize = L2_LOG_WQ_DATA_ENTRY_BSIZE + L2_LOG_SQ_RING_DEPTH;
        result =
            allocate_sq_memory(app_cfg->flexio_process, L2_LOG_SQ_RING_DEPTH,
                               log_sqd_bsize, &app_cfg->sq_transf[i]);
        if (result != DOCA_SUCCESS)
            return result;

        sq_attr.wq_ring_qmem.daddr = app_cfg->sq_transf[i].wq_ring_daddr;

        ret = flexio_sq_create(app_cfg->flexio_process, NULL, cq_num, &sq_attr,
                               &app_cfg->flexio_sq_ptr[i]);
        if (ret != FLEXIO_STATUS_SUCCESS) {
            DOCA_LOG_ERR("Failed to create FlexIO SQ");
            return DOCA_ERROR_DRIVER;
        }
        app_cfg->sq_transf[i].wq_num =
            flexio_sq_get_wq_num(app_cfg->flexio_sq_ptr[i]);
        /* Create SQ TX MKey */
        result = create_dpa_mkey(app_cfg->flexio_process, app_cfg->pd,
                                 app_cfg->sq_transf[i].wqd_daddr, log_sqd_bsize,
                                 IBV_ACCESS_LOCAL_WRITE, &app_cfg->sqd_mkey[i]);
        if (result != DOCA_SUCCESS)
            return result;

        app_cfg->sq_transf[i].wqd_mkey_id =
            flexio_mkey_get_id(app_cfg->sqd_mkey[i]);
    }
...
static doca_error_t init_dpa_rq_ring(struct flexio_process *process,
                                     flexio_uintptr_t ring_daddr, int log_depth,
...
static doca_error_t allocate_rq(struct l2_reflector_config *app_cfg) {
    ...
    for (int i = 0; i < app_cfg->nb_workers; i++) {
        ...
        struct flexio_cq_attr rqcq_attr = {
            ...
            .thread =
                flexio_event_handler_get_thread(app_cfg->event_handler[i]),
            .uar_id = flexio_uar_get_id(app_cfg->flexio_uar),
            .uar_base_addr = 0};
        /* RQ attributes */
        struct flexio_wq_attr rq_attr = {.log_wq_depth = L2_LOG_RQ_RING_DEPTH,
                                         .pd = app_cfg->pd};

        /* Allocate memory for RQ's CQ */
        result =
            allocate_cq_memory(app_cfg->flexio_process, L2_LOG_CQ_RING_DEPTH,
                               &app_cfg->rq_cq_transf[i]);
        if (result != DOCA_SUCCESS)
            return result;

        rqcq_attr.cq_dbr_daddr = app_cfg->rq_cq_transf[i].cq_dbr_daddr;
        rqcq_attr.cq_ring_qmem.daddr = app_cfg->rq_cq_transf[i].cq_ring_daddr;

        /* Create CQ and RQ */
        ret = flexio_cq_create(app_cfg->flexio_process, NULL, &rqcq_attr,
                               &app_cfg->flexio_rq_cq_ptr[i]);
        if (ret != FLEXIO_STATUS_SUCCESS) {
            DOCA_LOG_ERR("Failed to create FlexIO RQ's CQ");
            return DOCA_ERROR_DRIVER;
        }

        cq_num = flexio_cq_get_cq_num(app_cfg->flexio_rq_cq_ptr[i]);
        app_cfg->rq_cq_transf[i].cq_num = cq_num;
        app_cfg->rq_cq_transf[i].log_cq_depth = L2_LOG_RQ_RING_DEPTH;

        log_rqd_bsize = L2_LOG_RQ_RING_DEPTH + L2_LOG_WQ_DATA_ENTRY_BSIZE;

        flexio_buf_dev_alloc(app_cfg->flexio_process, LOG2VALUE(log_rqd_bsize),
                             &app_cfg->rq_transf[i].wqd_daddr);
        if (app_cfg->rq_transf[i].wqd_daddr == 0) {
            DOCA_LOG_ERR("Failed to allocate memory for RQ data buffer");
            return DOCA_ERROR_DRIVER;
        }

        flexio_buf_dev_alloc(app_cfg->flexio_process,
                             LOG2VALUE(L2_LOG_CQ_RING_DEPTH) *
                                 sizeof(struct mlx5_wqe_data_seg),
                             &app_cfg->rq_transf[i].wq_ring_daddr);
        if (app_cfg->rq_transf[i].wq_ring_daddr == 0x0) {
            DOCA_LOG_ERR("Failed to allocate memory for RQ ring buffer");
            return DOCA_ERROR_DRIVER;
        }

        result = allocate_dbr(app_cfg->flexio_process,
                              &app_cfg->rq_transf[i].wq_dbr_daddr);
        if (result != DOCA_SUCCESS)
            return result;

        /* Create an MKey for RX buffer */
        result = create_dpa_mkey(app_cfg->flexio_process, app_cfg->pd,
                                 app_cfg->rq_transf[i].wqd_daddr, log_rqd_bsize,
                                 IBV_ACCESS_LOCAL_WRITE, &app_cfg->rqd_mkey[i]);
        if (result != DOCA_SUCCESS)
            return result;

        mkey_id = flexio_mkey_get_id(app_cfg->rqd_mkey[i]);

        result = init_dpa_rq_ring(
            app_cfg->flexio_process, app_cfg->rq_transf[i].wq_ring_daddr,
            L2_LOG_CQ_RING_DEPTH, app_cfg->rq_transf[i].wqd_daddr,
            L2_LOG_WQ_DATA_ENTRY_BSIZE, mkey_id);
        if (result != DOCA_SUCCESS)
            return result;

        rq_attr.wq_dbr_qmem.memtype = FLEXIO_MEMTYPE_DPA;
        rq_attr.wq_dbr_qmem.daddr = app_cfg->rq_transf[i].wq_dbr_daddr;
        rq_attr.wq_ring_qmem.daddr = app_cfg->rq_transf[i].wq_ring_daddr;

        ret = flexio_rq_create(app_cfg->flexio_process, NULL, cq_num, &rq_attr,
                               &app_cfg->flexio_rq_ptr[i]);
        if (ret != FLEXIO_STATUS_SUCCESS) {
            DOCA_LOG_ERR("Failed to create FlexIO SQ");
            return DOCA_ERROR_DRIVER;
        }

        wq_num = flexio_rq_get_wq_num(app_cfg->flexio_rq_ptr[i]);
        app_cfg->rq_transf[i].wqd_mkey_id = mkey_id;
        app_cfg->rq_transf[i].wq_num = wq_num;
        ...
        ret = flexio_host2dev_memcpy(app_cfg->flexio_process, dbr, sizeof(dbr),
                                     app_cfg->rq_transf[i].wq_dbr_daddr);
        if (ret != FLEXIO_STATUS_SUCCESS) {
...
static void l2_reflector_rq_destroy(struct l2_reflector_config *app_cfg) {
    ...
    for (int i = 0; i < app_cfg->nb_workers; i++) {
        ...
        ret |= flexio_buf_dev_free(app_cfg->flexio_process,
                                   app_cfg->rq_transf[i].wq_dbr_daddr);
        ret |= flexio_buf_dev_free(app_cfg->flexio_process,
                                   app_cfg->rq_transf[i].wq_ring_daddr);
        ret |= flexio_buf_dev_free(app_cfg->flexio_process,
...
static void l2_reflector_sq_destroy(struct l2_reflector_config *app_cfg) {
    ...
    for (int i = 0; i < app_cfg->nb_workers; i++) {
        ...
        ret |= flexio_buf_dev_free(app_cfg->flexio_process,
                                   app_cfg->sq_transf[i].wq_dbr_daddr);
        ret |= flexio_buf_dev_free(app_cfg->flexio_process,
                                   app_cfg->sq_transf[i].wq_ring_daddr);
        ret |= flexio_buf_dev_free(app_cfg->flexio_process,
                                   app_cfg->sq_transf[i].wqd_daddr);
...
static void dev_queues_destroy(struct l2_reflector_config *app_cfg) {
    ...
    for (int i = 0; i < app_cfg->nb_workers; i++) {
        l2_reflector_cq_destroy(app_cfg->flexio_process,
                                app_cfg->flexio_rq_cq_ptr[i],
                                app_cfg->rq_cq_transf[i]);
        l2_reflector_cq_destroy(app_cfg->flexio_process,
                                app_cfg->flexio_sq_cq_ptr[i],
                                app_cfg->sq_cq_transf[i]);
    }
}
...
l2_reflector_allocate_device_resources(struct l2_reflector_config *app_cfg) {
    ...
    for (int i = 0; i < app_cfg->nb_workers; i++) {
        ...
        app_cfg->dev_data[i] = (struct l2_reflector_data *)calloc(
            1, sizeof(*app_cfg->dev_data[i]));
        ...
        app_cfg->dev_data[i]->worker_id = i;

        ret = flexio_copy_from_host(
            app_cfg->flexio_process, app_cfg->dev_data[i],
            sizeof(*app_cfg->dev_data[i]), &app_cfg->dev_data_daddr[i]);
        if (ret != FLEXIO_STATUS_SUCCESS) {
            DOCA_LOG_ERR("Could not copy data to device");
            dev_queues_destroy(app_cfg);
            for (int j = 0; j < i; j++) {
                free(app_cfg->dev_data[j]);
            }
            return DOCA_ERROR_DRIVER;
        }
    }
...
create_flow_table(struct mlx5dv_dr_domain *domain, int level, int priority,
    ...
    tbl->dr_matcher = mlx5dv_dr_matcher_create(tbl->dr_table, priority,
                                               criteria_enable, match_mask);
...
l2_reflector_create_steering_rule_rx(struct l2_reflector_config *app_cfg) {
    ...
    match_mask =
        (struct mlx5dv_flow_match_parameters *)calloc(1, flow_match_size);
    if (match_mask == NULL) {
    ...
    app_cfg->rx_domain =
        mlx5dv_dr_domain_create(app_cfg->ibv_ctx, MLX5DV_DR_DOMAIN_TYPE_NIC_RX);
    ...
    /* Create rule */
    int i = 0;
    result = create_flow_table(app_cfg->rx_domain, 0, /* Table level */
                               0,                     /* Matcher priority */
                               match_mask, &app_cfg->rx_flow_table[0]);
    if (result != DOCA_SUCCESS) {
    ...
    for (i = 0; i < app_cfg->nb_workers; i++) {
        ...
        app_cfg->rx_rule[i]->dr_action = mlx5dv_dr_action_create_dest_devx_tir(
            flexio_rq_get_tir(app_cfg->flexio_rq_ptr[i]));
        ...
        DEVX_SET(dr_match_spec, match_mask->match_buf, smac_47_16,
                 (SRC_MAC + i) >> 16);
        DEVX_SET(dr_match_spec, match_mask->match_buf, smac_15_0,
                 (SRC_MAC + i) % (1 << 16));
        app_cfg->rx_rule[i]->dr_rule =
            mlx5dv_dr_rule_create(app_cfg->rx_flow_table[0]->dr_matcher,
                                  match_mask, actions_len, actions);
...
exit_with_error:
    free(match_mask);
    for (int j = 0; j < i; j++) {
        if (app_cfg->rx_rule[j]) {
            destroy_rule(app_cfg->rx_rule[j]);
            app_cfg->rx_rule[j] = NULL;
        }
    }
    destroy_table(app_cfg->rx_flow_table[0]);
    app_cfg->rx_flow_table[0] = NULL;
    mlx5dv_dr_domain_destroy(app_cfg->rx_domain);
    return result;
}
...
l2_reflector_create_steering_rule_tx(struct l2_reflector_config *app_cfg) {
    ...
    app_cfg->fdb_domain =
        mlx5dv_dr_domain_create(app_cfg->ibv_ctx, MLX5DV_DR_DOMAIN_TYPE_FDB);
    ...
    result = create_flow_table(app_cfg->fdb_domain, 0, /* Table level */
                               0,                      /* Matcher priority */
                               match_mask, &app_cfg->tx_flow_root_table);
    if (result != DOCA_SUCCESS) {
    ...
    result = create_flow_table(app_cfg->fdb_domain, 1, /* Table level */
                               0,                      /* Matcher priority */
                               match_mask, &app_cfg->tx_flow_table);
    if (result != DOCA_SUCCESS) {
    ...
    app_cfg->tx_root_rule->dr_action =
        mlx5dv_dr_action_create_dest_table(app_cfg->tx_flow_table->dr_table);
    ...
    app_cfg->tx_root_rule->dr_rule = mlx5dv_dr_rule_create(
        app_cfg->tx_flow_root_table->dr_matcher, match_mask, 1, actions);
    ...
    app_cfg->tx_rule->dr_action =
        mlx5dv_dr_action_create_dest_vport(app_cfg->fdb_domain, 0xFFFF);
    ...
    DEVX_SET(dr_match_spec, match_mask->match_buf, dmac_47_16, SRC_MAC >> 16);
    app_cfg->tx_rule->dr_rule = mlx5dv_dr_rule_create(
        app_cfg->tx_flow_table->dr_matcher, match_mask, 1, actions);
...
void l2_reflector_device_resources_destroy(
    struct l2_reflector_config *app_cfg) {
    dev_queues_destroy(app_cfg);
    for (int i = 0; i < app_cfg->nb_workers; i++) {
        flexio_buf_dev_free(app_cfg->flexio_process,
...
void l2_reflector_steering_rules_destroy(struct l2_reflector_config *app_cfg) {
    for (int i = 0; i < app_cfg->nb_workers; i++) {
        ...
        if (app_cfg->rx_flow_table[i]) {
            destroy_table(app_cfg->rx_flow_table[i]);
            app_cfg->rx_flow_table[i] = NULL;
        }
    }
    if (app_cfg->tx_rule) {
        destroy_rule(app_cfg->tx_rule);
...
void l2_reflector_destroy(struct l2_reflector_config *app_cfg) {
    ...
    if (flexio_process_destroy(app_cfg->flexio_process) !=
        FLEXIO_STATUS_SUCCESS)
...
static doca_error_t nb_workers_callback(void *param, void *config) {
    struct l2_reflector_config *app_cfg = (struct l2_reflector_config *)config;
    app_cfg->nb_workers = atoi((char *)param);
    return DOCA_SUCCESS;
}

doca_error_t register_l2_reflector_params(void) {
    struct doca_argp_param *device_param;
    struct doca_argp_param *workers_param;
    doca_error_t result;
    ...
    result = doca_argp_param_create(&workers_param);
    if (result != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Failed to create ARGP param: %s",
                     doca_error_get_descr(result));
        return result;
    }
    doca_argp_param_set_short_name(workers_param, "w");
    doca_argp_param_set_long_name(workers_param, "workers");
    doca_argp_param_set_arguments(workers_param, "<nb workers>");
    doca_argp_param_set_description(workers_param, "Nb workers");
    doca_argp_param_set_callback(workers_param, nb_workers_callback);
    doca_argp_param_set_type(workers_param, DOCA_ARGP_TYPE_STRING);
    result = doca_argp_register_param(workers_param);
    if (result != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Failed to register program param: %s",
                     doca_error_get_descr(result));
        return result;
