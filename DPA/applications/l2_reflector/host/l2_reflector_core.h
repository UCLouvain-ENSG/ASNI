...
struct dr_flow_table {
    struct mlx5dv_dr_table
        *dr_table; /* DR table in the domain at specific level */
    struct mlx5dv_dr_matcher
        *dr_matcher; /* DR matcher object in the table. One matcher per table */
};
...
struct l2_reflector_config {
    ...
    struct l2_reflector_data *dev_data[MAX_NB_WORKERS]; /* device data */
    int nb_workers;                                  /* Number of workers */
    ...
    flexio_uintptr_t dev_data_daddr[MAX_NB_WORKERS]; /* Data address accessible
                                                        by the device */
    struct flexio_process *flexio_process;           /* FlexIO process */
    struct flexio_uar *flexio_uar;                   /* FlexIO UAR */
    struct flexio_event_handler
        *event_handler[MAX_NB_WORKERS]; /* Event handler on device */

    struct app_transfer_cq rq_cq_transf[MAX_NB_WORKERS];
    struct app_transfer_cq sq_cq_transf[MAX_NB_WORKERS];

    struct flexio_mkey *rqd_mkey[MAX_NB_WORKERS];
    struct app_transfer_wq rq_transf[MAX_NB_WORKERS];

    struct flexio_mkey *sqd_mkey[MAX_NB_WORKERS];
    struct app_transfer_wq sq_transf[MAX_NB_WORKERS];

    struct flexio_cq *flexio_rq_cq_ptr[MAX_NB_WORKERS]; /* FlexIO RQ CQ */
    struct flexio_cq *flexio_sq_cq_ptr[MAX_NB_WORKERS]; /* FlexIO SQ CQ */
    struct flexio_rq *flexio_rq_ptr[MAX_NB_WORKERS];    /* FlexIO RQ */
    struct flexio_sq *flexio_sq_ptr[MAX_NB_WORKERS];    /* FlexIO SQ */

    /* mlx5dv direct rules resources, used for steering rules */
    struct mlx5dv_dr_domain *rx_domain;
    struct mlx5dv_dr_domain *fdb_domain;

    struct dr_flow_table *rx_flow_table[MAX_NB_WORKERS];
    struct dr_flow_table *tx_flow_table;
    struct dr_flow_table *tx_flow_root_table;

    struct dr_flow_rule *rx_rule[MAX_NB_WORKERS];
