...
#define MAX_NB_WORKERS 256

flexio_dev_rpc_handler_t
    l2_reflector_dev_init; /* Device initialization function */
flexio_dev_event_handler_t
    l2_reflector_event_handler; /* Event handler function */
...
/* Device context */
static struct {
    uint32_t lkey[MAX_NB_WORKERS];            /* Local memory key */
    uint32_t is_initalized[MAX_NB_WORKERS];   /* Initialization flag */
    struct cq_ctx_t rqcq_ctx[MAX_NB_WORKERS]; /* RQ CQ context */
    struct cq_ctx_t sqcq_ctx[MAX_NB_WORKERS]; /* SQ CQ context */
    struct rq_ctx_t rq_ctx[MAX_NB_WORKERS];   /* RQ context */
    struct sq_ctx_t sq_ctx[MAX_NB_WORKERS];   /* SQ context */
    struct dt_ctx_t dt_ctx[MAX_NB_WORKERS];   /* DT context */
    uint32_t packets_count[MAX_NB_WORKERS];   /* Number of processed packets */
} dev_ctx = {0};
 ...
 */
static void *get_next_dte(struct dt_ctx_t *dt_ctx, uint32_t dt_idx_mask,
...
/*
 * This is the main function of the L2 reflector device, called on each packet
 * from l2_reflector_device_event_handler() Packet are received from the RQ,
 * processed by changing MAC addresses and transmitted to the SQ.
 *
 * @dtctx [in]: This thread context
 */
static void process_packet(struct flexio_dev_thread_ctx *dtctx, int worker_id) {
    uint32_t rq_wqe_idx;
    ...
    rq_wqe_idx =
        flexio_dev_cqe_get_wqe_counter(dev_ctx.rqcq_ctx[worker_id].cqe);
    data_sz = flexio_dev_cqe_get_byte_cnt(dev_ctx.rqcq_ctx[worker_id].cqe);

    /* Get RQ WQE pointed by CQE */
    rwqe = &dev_ctx.rq_ctx[worker_id].rq_ring[rq_wqe_idx & L2_RQ_IDX_MASK];

    /* Extract data (whole packet) pointed by RQ WQE */
    rq_data = flexio_dev_rwqe_get_addr(rwqe);

    /* Take next entry from data ring */
    sq_data = get_next_dte(&dev_ctx.dt_ctx[worker_id], L2_DATA_IDX_MASK,
    ...
    // char *dst_mac = sq_data;
    // flexio_dev_print("mac address: %02x:%02x:%02x:%02x:%02x:%02x\n", dst_mac[0],
    //                  dst_mac[1], dst_mac[2], dst_mac[3], dst_mac[4], dst_mac[5]);

    /* Take first segment for SQ WQE (3 segments will be used) */
    swqe = get_next_sqe(&dev_ctx.sq_ctx[worker_id], L2_SQ_IDX_MASK);

    /* Fill out 1-st segment (Control) */
    flexio_dev_swqe_seg_ctrl_set(swqe, dev_ctx.sq_ctx[worker_id].sq_pi,
                                 dev_ctx.sq_ctx[worker_id].sq_number,
    ...
    swqe = get_next_sqe(&dev_ctx.sq_ctx[worker_id], L2_SQ_IDX_MASK);
    flexio_dev_swqe_seg_mem_ptr_data_set(swqe, data_sz, dev_ctx.lkey[worker_id],
                                         (uint64_t)sq_data);
    ...
    /* Ring DB */
    __dpa_thread_fence(__DPA_MEMORY, __DPA_W, __DPA_W);
    dev_ctx.sq_ctx[worker_id].sq_pi++;
    flexio_dev_qp_sq_ring_db(dtctx, dev_ctx.sq_ctx[worker_id].sq_pi,
                             dev_ctx.sq_ctx[worker_id].sq_number);
    __dpa_thread_fence(__DPA_MEMORY, __DPA_W, __DPA_W);
    flexio_dev_dbr_rq_inc_pi(dev_ctx.rq_ctx[worker_id].rq_dbr);
}
...
__dpa_rpc__ uint64_t l2_reflector_device_init(uint64_t data) {
    ...
    int worker_id = shared_data->worker_id;
    flexio_dev_print("Initializing worker : %d\n", worker_id);
    dev_ctx.lkey[worker_id] = shared_data->sq_data.wqd_mkey_id;
    init_cq(shared_data->rq_cq_data, &dev_ctx.rqcq_ctx[worker_id]);
    init_rq(shared_data->rq_data, &dev_ctx.rq_ctx[worker_id]);
    init_cq(shared_data->sq_cq_data, &dev_ctx.sqcq_ctx[worker_id]);
    init_sq(shared_data->sq_data, &dev_ctx.sq_ctx[worker_id]);

    dev_ctx.dt_ctx[worker_id].sq_tx_buff =
        (void *)shared_data->sq_data.wqd_daddr;
    dev_ctx.dt_ctx[worker_id].tx_buff_idx = 0;
    dev_ctx.is_initalized[worker_id] = 1;
    return 0;
}

/*
 * This function is called when a new packet is received to RQ's CQ.
 * Upon receiving a packet, the function will iterate over all received packets
 * and process them. Once all packets in the CQ are processed, the CQ will be
 * rearmed to receive new packets events.
 */
void __dpa_global__ l2_reflector_device_event_handler(uint64_t arg0) {
    struct flexio_dev_thread_ctx *dtctx;
    int worker_id = arg0;
    flexio_dev_get_thread_ctx(&dtctx);
    if (dev_ctx.is_initalized[worker_id] == 0)
        flexio_dev_thread_reschedule();

    // flexio_dev_print("Handler : %d\n", worker_id);
    while (flexio_dev_cqe_get_owner(dev_ctx.rqcq_ctx[worker_id].cqe) !=
           dev_ctx.rqcq_ctx[worker_id].cq_hw_owner_bit) {
        __dpa_thread_fence(__DPA_MEMORY, __DPA_R, __DPA_R);
        // flexio_dev_print("Process packet : %ld\n", arg0);
        process_packet(dtctx, worker_id);
        step_cq(&dev_ctx.rqcq_ctx[worker_id], L2_CQ_IDX_MASK);
    }
    __dpa_thread_fence(__DPA_MEMORY, __DPA_W, __DPA_W);
    flexio_dev_cq_arm(dtctx, dev_ctx.rqcq_ctx[worker_id].cq_idx,
                      dev_ctx.rqcq_ctx[worker_id].cq_number);
    flexio_dev_thread_reschedule();
}
