...
always_inline void xchg_tx_sent(struct rte_mbuf **elts, struct xchg **xchgs) {
    ...
    // We can put random stuff here apparently which is cool
    *elts = NULL;
}
...
always_inline bool xchg_get_buffer_len(struct xchg *xchg) {
    if (likely(*xchg->buffer)) {
        ...
        if (likely(*(xchg->buffer + asni_header_size +
                     (*xchg->buffer * sizeof(struct descriptor))) == 0x78)) {
            // printf("BUF %p Marker at %lu ok !\n",xchg->buffer,
            // asni_header_size +  (*xchg->buffer * sizeof(struct descriptor) ));
            *(xchg->buffer + asni_header_size +
              (*xchg->buffer * sizeof(struct descriptor))) = 0;
            return true;
        } else {
            // printf("BUF %p Marker at %lu not ok : %x !\n",xchg->buffer,
            // asni_header_size +  (*xchg->buffer * sizeof(struct descriptor) ),
            // *(xchg->buffer +  asni_header_size + (*xchg->buffer *
 ...
 */
always_inline bool xchg_get_buffer_len(struct xchg *xchg) { return xchg->plen; }
...
// a few Click stuffs.
always_inline void xchg_rx_finish_packet(struct xchg *xchg) {
    //#ifndef NO_PREFETCH_PAYLOAD  //THIS IS ONLY FOR SUBPACKET
...
//#ifndef NO_PREFETCH_PAYLOAD //We do need to prefetch ASNI headers...
...
//#endif
        ...
        if (unlikely(first->buffer == 0)) {
            ...
            if (!fresh_buf) {
                rte_exit(EXIT_FAILURE,"failed to allocate buffer\n");
            }
        } else {
