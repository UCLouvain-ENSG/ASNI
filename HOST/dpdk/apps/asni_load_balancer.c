...
static void signal_handler(int signum) {
    if (signum == SIGINT || signum == SIGTERM || signum == SIGKILL) {
        ...
        printf("\nNumber of received packets : %ld"
               ...
               new_stats.q_errors[0], new_stats.ierrors);
...
static int lcore_main(uint16_t port) {
    ...
    printf("\nCore %u counting incoming packets from queue %d, port %d. "
           "[Ctrl+C to quit]\n",
    ...
    for (;;) {
        if (force_quit) {
            ...
            printf("\nReceived %d packets in %f seconds : throughput : %fGb/s\n"
                   ...
                   (nb_byte * 8 / time_elapsed) / 1000000000, rte_burst,
                   pkt_analysis);
            ...
            fprintf(fp, "%ld, %ld, %f, %f, %ld, %ld, %f, %f\n", counter,
                    counter, time_elapsed,
                    (nb_byte * 8 / time_elapsed) / 1000000000, rte_burst,
                    pkt_analysis, counter / time_elapsed / 1000000,
                    counter / time_elapsed / 1000000);
            fclose(fp);
        ...
        for (int i = 0; i < nb_rx; i++) {
            total_length = bufs[i]->pkt_len;
            // printf("total_length : %d\n", total_length);
            curr_tot = 0;
            ch =
                (struct custom_header *)(rte_pktmbuf_mtod(bufs[index], char *));
            // printf("=================\n");
        ...
        //         ip_hdr = rte_pktmbuf_mtod_offset(bufs[index], struct
        //         rte_ipv4_hdr *, sizeof(struct rte_ether_hdr));
        ...
        // if (pkts_sent != nb_rx) {
        //     rte_pktmbuf_free_bulk(&(bufs[pkts_sent]), (nb_rx - pkts_sent) +
        //     1);
        // }
...
int main(int argc, char *argv[]) {
    ...
    if (large_mtu_port_init(portid, mbuf_pool, 1, 1, 9800) != 0)
