...
static int lcore_main(void *arg) {
    ...
    for (;;) {
        ...
        for (int i = 0; i < nb_rx; i++) {
            nb_byte += pkts_burst[i]->plen;
            pkts_tx_burst[i]->buffer = pkts_burst[i]->buffer;
            pkts_tx_burst[i]->plen = pkts_burst[i]->plen;
        }
        const uint16_t sent =
            rte_eth_tx_burst_xchg(dst_port, qid, pkts_tx_burst, nb_rx);
...
#else
        const uint16_t sent = rte_eth_tx_burst(dst_port, 0, pkts_burst, nb_rx);
