...
always_inline void xchg_rx_finish_packet(struct xchg* xchg) { }
 ...
 always_inline bool xchg_get_buffer_len(struct xchg* xchg) {
    if (likely(*xchg->buffer)) {
        ...
        if (likely(*(xchg->buffer + asq_header_size + (*xchg->buffer * sizeof(struct descriptor) )) == 0x78 )) {
            //printf("BUF %p Marker at %lu ok !\n",xchg->buffer, asq_header_size +  (*xchg->buffer * sizeof(struct descriptor) ));
            *(xchg->buffer + asq_header_size + (*xchg->buffer * sizeof(struct descriptor) )) = 0;
            return true;
        } else {
            //printf("BUF %p Marker at %lu not ok : %x !\n",xchg->buffer, asq_header_size +  (*xchg->buffer * sizeof(struct descriptor) ), *(xchg->buffer +  asq_header_size + (*xchg->buffer * sizeof(struct descriptor) )));
        }
    }
    return false;
 }
