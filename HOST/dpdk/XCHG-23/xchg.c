...
#ifdef HAVE_RTC
always_inline bool xchg_rtc_only() {
        return true;
}
#else
always_inline bool xchg_rtc_only() {
...
always_inline bool xchg_read_len() {
        return false;
}
always_inline bool xchg_need_hash() {
        return false;
}
...
always_inline void xchg_rx_finish_packet(struct xchg* xchg) { }
 ...
 always_inline bool xchg_get_buffer_len(struct xchg* xchg) {
    if (likely(*xchg->buffer)) {
        ...
        if (likely(*(xchg->buffer + asni_header_size + (*xchg->buffer * sizeof(struct descriptor) )) == 0x78 )) {
            //printf("BUF %p Marker at %lu ok !\n",xchg->buffer, asni_header_size +  (*xchg->buffer * sizeof(struct descriptor) ));
            *(xchg->buffer + asni_header_size + (*xchg->buffer * sizeof(struct descriptor) )) = 0;
            return true;
        } else {
            //printf("BUF %p Marker at %lu not ok : %x !\n",xchg->buffer, asni_header_size +  (*xchg->buffer * sizeof(struct descriptor) ), *(xchg->buffer +  asni_header_size + (*xchg->buffer * sizeof(struct descriptor) )));
        }
    }
    return false;
 }
...
always_inline bool xchg_need_hash() {
 ...
 always_inline bool xchg_need_vlan() { return true; };
     ...
     */
    always_inline struct xchg* xchg_rx_next(struct rte_mbuf** rep, struct xchg** xchgs, struct rte_mempool* mp, int *advance_consumer) {
        if (xchg_rtc_only()) {
    ...
    /**
     * Cancel the current receiving, this should cancel the last xchg_next, ie put back the XCHG buffer address to what is still in rep
     *
     * Scenario where XCHG was empty
     * XCHGS buffers[ 0, 0, 0, 0]
     * ELTS [A, B, C, D]
     * REP = 0;
     * RX
     * -> repleneished
     ...
     * [Z,0,0,0]
     */
    always_inline void xchg_rx_cancel(struct xchg* xchg, struct rte_mbuf* rep, int *advance_consumer) {
        if (!xchg_rtc_only()) {
