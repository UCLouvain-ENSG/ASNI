...
#if  defined(ASNI_DESC_PAD) && defined(ASNI_MODE_DPDK_BASELINE)
...
// #        return retval;
    ...
    static const struct rte_mbuf_dynfield dynfield_desc = {
        ...
        .size = ASNI_DESC_PAD > 32? sizeof(void*) : ASNI_DESC_PAD ,
        .align = 4,
    };
    ...
    if (ASNI_DESC_PAD > 32) {
        pad_pool = rte_mempool_create("pad_pool", 2048, ASNI_DESC_PAD, 32, 0, 0, 0,0 ,0 ,0 ,0);
        if (!pad_pool) {
