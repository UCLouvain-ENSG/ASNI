...
int nb_core = 1;
int light = 0;
enum State { NO_ASNI, ASNI, HW_ASNI_DP, HW_ASNI_DD };

enum State server_version = NO_ASNI;
...
static inline int port_init(uint16_t port, struct rte_mempool *mbuf_pool) {
    ...
    if (server_version == ASNI || server_version == HW_ASNI_DP ||
        server_version == HW_ASNI_DD) {
...
lcore_main(void* arg) {
    ...
    for (;;) {
        ...
        for (index = 0; index < nb_rx; index++) {
            ...
            if (server_version != HW_ASNI_DP && server_version != HW_ASNI_DD) {
                data = rte_pktmbuf_mtod(bufs[index], uint8_t *);
            }
            if (light)
                rte_prefetch0(data);
            if (server_version == ASNI) {
                debug_printf("Inside ASNI\n", NULL);
                offset_desc = 16;
                nb_desc = *data;
            ...
            } else if (server_version == HW_ASNI_DP) {
                debug_printf("inside hw_asni_dp\n", NULL);
                ...
                rte_pktmbuf_free(bufs[index]);
            } else if (server_version == HW_ASNI_DD) {
                debug_printf("inside hw_asni_dd\n", NULL);
                uint32_t total_length = bufs[index]->pkt_len;
                uint32_t curr_tot = 0;
                uint64_t dummy_counter;
                struct custom_header *ch;
...
int option(int argc, char **argv) {
    ...
    while (1) {
        static struct option long_options[] = {
            ...
            {"asni", no_argument, (int *)&server_version, ASNI},
            {"hw_asni_dp", no_argument, (int *)&server_version, HW_ASNI_DP},
            {"hw_asni_dd", no_argument, (int *)&server_version, HW_ASNI_DD},
