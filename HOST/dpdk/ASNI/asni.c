...
int option(int argc, char **argv, struct asni_state *asni_state) {
    ...
    while (1) {
        ...
        case 's':
            s = 0;
            asni_state->macAddr1.addr_bytes[0] = (int)strtol(optarg, NULL, 16);
            asni_state->macAddr1.addr_bytes[1] =
                (int)strtol(optarg + 3, NULL, 16);
            asni_state->macAddr1.addr_bytes[2] =
                (int)strtol(optarg + 6, NULL, 16);
            asni_state->macAddr1.addr_bytes[3] =
                (int)strtol(optarg + 9, NULL, 16);
            asni_state->macAddr1.addr_bytes[4] =
                (int)strtol(optarg + 12, NULL, 16);
            asni_state->macAddr1.addr_bytes[5] =
    ...
    asni_state->nb_core = rte_lcore_count() - 1;
    asni_state->light = light;
...
int option_dpt(int argc, char **argv, struct asni_state *state,
               char *pci_addr_str) {
...
struct asni_state *asni_init_dpt(int argc, char **argv) {
    printf("Running ASNI with descriptors of size %lu\n",
    ...
    struct asni_state *state =
        (struct asni_state *)malloc(sizeof(struct asni_state));
    ...
    argv += ret;
    if (state == NULL) {
        printf("Error allocating memory for asni_state\n");
        return NULL;
    }
...
struct asni_state *asni_init(int argc, char **argv) {
    printf("Running ASNI with descriptors of size %lu\n",
           sizeof(struct descriptor));
    struct asni_state *state = malloc(sizeof(struct asni_state));
    struct rte_mempool *mbuf_pool;
    uint16_t portid = 0;
    state->port = 0;
    state->light = 0;
...
struct asni_iterator *asni_init_iterator(uint16_t burst_size) {
    struct asni_iterator *iterator = calloc(1, sizeof(struct asni_iterator));
