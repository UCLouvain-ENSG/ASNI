...
/**
 * @brief Initializes ASNI environment.
 *
 * @param argc [in]: command line arguments size
 * @param argv [in]: array of command line arguments
 *
 * @return ASNI state containing everything needed to use receive descriptors
 */
struct asni_state *asni_init(int argc, char **argv);

struct asni_state *asni_init_dpt(int argc, char **argv);
/**
 * Runs the application.
 *
 * @param state [in]: ASNI state
 * @param app [in]: function ptr towards the application to run
 */
void asni_run(void *state, int (*app)(void *));
 ...
 */
void asni_rx(struct asni_state *state, struct asni_iterator *iterator);

struct asni_iterator *asni_init_iterator(uint16_t burst_size);

struct descriptor *asni_get_next(struct asni_iterator *iterator);
...
#define ASNI_FOR_EACH_PACKET_DP(state, descriptor, force_quit)                 \
  printf("inside for each sw\n");                                              \
  struct asni_state *asni_state = (struct asni_state *)state;                  \
  ...
  while (!*force_quit) {                                                       \
    START_CYCLE();                                                             \
    uint16_t nb_rx = rte_eth_rx_burst(asni_state->port, rte_lcore_id() - 1,    \
...
#define ASNI_FOR_EACH_PACKET_DP_END()                                          \
...
#define ASNI_HW_DP_FOR_EACH_PACKET(state, descriptor, force_quit)              \
      ...
      if (unlikely(nb_desc < 1 || nb_desc > 64 || magic != MAGIC)) {           \
        ...
        for (uint8_t j = 0; j < nb_desc; j++) {                                \
          ...
          struct descriptor *next_descriptor =                                 \
              (struct descriptor *)(((char *)(descriptor + 1)) + length);      \
          char *next_payload = (char *)(next_descriptor + 1);                  \
          rte_prefetch0(next_descriptor);                                      \
          rte_prefetch0(next_payload);

#define ASNI_HW_DP_FOR_EACH_PACKET_END()                                       \
  descriptor = next_descriptor;                                                \
  payload = next_payload;                                                      \
  (void)payload;                                                               \
  }                                                                            \
...
#define ASNI_FOR_EACH_PACKET_PP(state, descriptor, force_quit)                 \
    ...
    for (int i = 0; i < nb_rx; i++) {                                          \
      ...
      if (unlikely(nb_desc < 1 || nb_desc > 64 || magic != MAGIC)) {           \
        ...
        for (uint8_t j = 0; j < nb_desc; j++) {                                \
          ...
          int length =                                                         \
              sizeof(struct rte_ether_hdr) + htons(ip_hdr->total_length);      \
          char *next_payload = payload + length;                               \
          rte_prefetch0(next_payload);                                         \
          descriptor->size = length;

#define ASNI_FOR_EACH_PACKET_PP_END()                                          \
  payload = next_payload;                                                      \
  (void)payload;                                                               \
  }                                                                            \
  PROCESSED_PACKETS();                                                         \
...
#define ASNI_FOR_EACH_PACKET_PP_EXP_DESC(state, descriptor, force_quit)        \
...
#define ASNI_FOR_EACH_PACKET_PP_EXP_DESC_END()                                 \
  (void)payload;                                                               \
  }                                                                            \
  PROCESSED_PACKETS();                                                         \
  }                                                                            \
...
typedef struct descriptor asni_descriptor_t;

#endif

#define PROCESS_BURST_ASNI(port, queue, batch_size, descriptors, rx_count,     \
                           state, metadata, payload_head)                      \
  uint8_t looping = 1;                                                         \
  uint16_t io_rx_count = 0;                                                    \
  void **io_descriptors;                                                       \
  while (looping) {                                                            \
    ASNI_IO_RX_BURST(port, queue, fake_dpdk_io_descriptors, batch_size,        \
                     fake_dpdk_io_rx_count, state);                            \
  ...
  uint32_t offset_desc = DESC_OFFSET;                                          \
  for (int asni_packet_index = 0; asni_packet_index < io_rx_count;             \
       asni_packet_index++) {                                                  \
    uint8_t *data;                                                             \
    ASNI_IO_GET_PAYLOAD_PTR(io_descriptors[asni_packet_index], data);          \
    uint8_t rx_count = *data;                                                  \
    descriptors = data + offset_desc;                                          \
    payload_head = data + offset_desc + rx_count * sizeof(struct descriptor);  \
    ASNI_IO_SETUP_METADATA(io_descriptors[asni_packet_index], rx_count,        \
                           metadata);

#define PROCESS_BURST_ASNI_END()                                               \
  ASNI_IO_END_PROCESS(io_descriptors[asni_packet_index]);                      \
  }

#define PROCESS_BURST_PAYLOAD_NEXT_ASNI(descriptor, payload_head)              \
  payload_head += descriptor.size;

#define PROCESS_BURST_GET_PAYLOAD_ASNI(descriptor, payload, payload_head)      \
  payload = payload_head;

#ifdef ASNI_DESC_ETH_TYPE
#define PROCESS_BURST_GET_ETH_TYPE_ASNI(descriptor, eth_type, payload)         \
...
#define PROCESS_BURST_GET_IP_SRC_ASNI(descriptor, ip_src, payload)             \
...
#define PROCESS_BURST_GET_IP_DST_ASNI(descriptor, ip_dst, payload)             \
...
#define PROCESS_BURST_GET_PORT_SRC_ASNI(descriptor, port_src, payload)         \
...
#define PROCESS_BURST_GET_PORT_DST_ASNI(descriptor, port_dst, payload)         \
...
#define PROCESS_BURST_GET_IP_PROTO_ASNI(descriptor, ip_proto, payload)         \
  ip_proto = descriptor.ip_proto;
#else
#define PROCESS_BURST_GET_IP_PROTO_ASNI(descriptor, ip_proto, payload)         \
  ip_proto = ((struct rte_ipv4_hdr *)(payload + sizeof(struct rte_ether_hdr))) \
                 ->next_proto_id;
#endif

#define PROCESS_BURST_GET_SIZE_ASNI(descriptor, size) size = descriptor.size;

#define PROCESS_BURST_GET_IMPLICIT_SIZE_ASNI(descriptor, size)                 \
...
#define PROCESS_BURST(port, queue, batch_size, descriptors, rx_count, state,   \
  ...
  PROCESS_BURST_ASNI(port, queue, batch_size, descriptors, rx_count, state,    \
                     metadata, payload_head)
#define PROCESS_BURST_END() PROCESS_BURST_ASNI_END()
#define PROCESS_BURST_PAYLOAD_NEXT(descriptor, payload_head)                   \
  PROCESS_BURST_PAYLOAD_NEXT_ASNI(descriptor, payload_head)
#define PROCESS_BURST_GET_PAYLOAD(descriptor, payload, payload_head)           \
  PROCESS_BURST_GET_PAYLOAD_ASNI(descriptor, payload, payload_head)
#define PROCESS_BURST_GET_ETH_TYPE(descriptor, eth_type, payload)              \
  PROCESS_BURST_GET_ETH_TYPE_ASNI(descriptor, eth_type, payload)
#define PROCESS_BURST_GET_IP_SRC(descriptor, ip_src, payload)                  \
  PROCESS_BURST_GET_IP_SRC_ASNI(descriptor, ip_src, payload)
#define PROCESS_BURST_GET_IP_DST(descriptor, ip_dst, payload)                  \
  PROCESS_BURST_GET_IP_DST_ASNI(descriptor, ip_dst, payload)
#define PROCESS_BURST_GET_PORT_SRC(descriptor, port_src, payload)              \
  PROCESS_BURST_GET_PORT_SRC_ASNI(descriptor, port_src, payload)
#define PROCESS_BURST_GET_PORT_DST(descriptor, port_dst, payload)              \
  PROCESS_BURST_GET_PORT_DST_ASNI(descriptor, port_dst, payload)
#define PROCESS_BURST_GET_IP_PROTO(descriptor, ip_proto, payload)              \
  PROCESS_BURST_GET_IP_PROTO_ASNI(descriptor, ip_proto, payload)
#define PROCESS_BURST_GET_SIZE(descriptor, size)                               \
  PROCESS_BURST_GET_SIZE_ASNI(descriptor, size)
#define PROCESS_BURST_GET_IMPLICIT_SIZE(descriptor, size)                      \
  PROCESS_BURST_GET_IMPLICIT_SIZE_ASNI(descriptor, size)

#endif

#ifdef ASNI_MODE_DPDK_CLASSIC

typedef struct rte_mbuf *asni_descriptor_t;
...
#define PROCESS_BURST(port, queue, batch_size, descriptors, rx_count, state,   \
                      metadata, payload_head)                                  \
  PROCESS_BURST_CLASSIC(port, queue, batch_size, descriptors, rx_count, state, \
                        metadata, payload_head)
...
#ifdef ASNI_MODE_OFFLOAD_TX
typedef struct descriptor asni_descriptor_t;
#endif

#define PROCESS_BURST_ASNI_OFFLOAD_TX(port, queue, batch_size, descriptors,    \
...
#define PROCESS_BURST_ASNI_OFFLOAD_TX_END()                                    \
  ASNI_IO_END_PROCESS(io_descriptors[asni_packet_index]);                      \
  }

#define PROCESS_BURST_PAYLOAD_NEXT_ASNI_OFFLOAD_TX(descriptor, payload_head)   \
  payload_head += descriptor.size;

#define PROCESS_BURST_GET_PAYLOAD_ASNI_OFFLOAD_TX(descriptor, payload,         \
                                                  payload_head)                \
  payload = payload_head;
...
#define PROCESS_BURST_GET_ETH_TYPE_ASNI(descriptor, eth_type, payload)         \
  eth_type = descriptor.eth_type;
#else
#define PROCESS_BURST_GET_ETH_TYPE_ASNI_OFFLOAD_TX(descriptor, eth_type,       \
                                                   payload)                    \
  eth_type = rte_be_to_cpu_16(((struct rte_ether_hdr *)payload)->ether_type);
#endif

#ifdef ASNI_DESC_IP_SRC
#define PROCESS_BURST_GET_IP_SRC_ASNI_OFFLOAD_TX(descriptor, ip_src, payload)  \
  ip_src = descriptor.ip_src;
#else
#define PROCESS_BURST_GET_IP_SRC_ASNI(descriptor, ip_src, payload)             \
  ip_src = ((struct rte_ipv4_hdr *)(payload + sizeof(struct rte_ether_hdr)))   \
...
#define PROCESS_BURST(port, queue, batch_size, descriptors, rx_count, state,   \
                      metadata, payload_head)                                  \
  PROCESS_BURST_ASNI_OFFLOAD_TX(port, queue, batch_size, descriptors,          \
                                rx_count, state, metadata, payload_head)
#define PROCESS_BURST_END() PROCESS_BURST_ASNI_OFFLOAD_TX_END()
#define PROCESS_BURST_PAYLOAD_NEXT(descriptor, payload_head)                   \
  PROCESS_BURST_PAYLOAD_NEXT_ASNI(descriptor, payload_head)
#define PROCESS_BURST_GET_PAYLOAD(descriptor, payload, payload_head)           \
  payload = payload_head;
