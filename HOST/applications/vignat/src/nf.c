...
#if defined(WITH_ASNI) && defined(ASNI_IO_DPDK)
#error "ASNI is not supported with DPDK IO yet"
#endif
        ...
        for (uint16_t device = 0; device < VIGOR_DEVICES_COUNT; device++) {
            ...
            for (uint16_t n = 0; n < rx_count; n++) {
                ...
                if (dst_device != EXPLICIT_DROP
                    ...
                    ASNI_IO_TX_ENQUEUE(state, descriptors[n], size,
                                            metadata, payload);
                } else {
            ...
            ASNI_IO_TX_BURST(ports[1 - device], 0, rx_count, tx_count,
                                  state);
            PROCESS_BURST_END();
            ASNI_IO_TX_BURST_LATE(ports[1 - device], 0, 0, 0, state);
        }
    }
}
