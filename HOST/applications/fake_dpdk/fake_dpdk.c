...
void fake_dpdk_init(int argc, char **argv, uint8_t nb_core, uint8_t burst_size,
    ...
    // Run the application
#ifndef ASNI_BURST_MODE
    (*app)(&fake_state);
    stable_dma_dpdk_free(
        (struct stable_dma_dpdk_dma_state *)fake_state.underlying_state);
#endif /* ifndef ASNI_BURST_MODE */
#endif
#if defined ASNI_MODE ||                                         \
    defined ASNI_MODE_HW_DP ||                                   \
    defined ASNI_MODE_HW_DD
    fake_state.underlying_state = asni_init(argc, argv);
    asni_run(&fake_state, app);
#endif
#ifdef ASNI_MODE_DPT
    fake_state.underlying_state = asni_init_dpt(argc, argv);
    asni_run(&fake_state, app);
#endif
