...
#define RTE_HUGE_MBUF_SIZE 9512
...
#if defined(FAKE_DPDK_IO_DPDK) || defined(FAKE_DPDK_IO_ASQ_OFFLOAD_TX)
        port_conf.txmode.offloads |=
            RTE_ETH_TX_OFFLOAD_MBUF_FAST_FREE | RTE_ETH_TX_OFFLOAD_IPV4_CKSUM |
            RTE_ETH_TX_OFFLOAD_UDP_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_CKSUM;
#endif
#ifdef FAKE_DPDK_IO_XCHG
    port_conf.txmode.offloads |=
        RTE_ETH_TX_OFFLOAD_MBUF_FAST_FREE | DEV_TX_OFFLOAD_IPV4_CKSUM |
...
// In case DPDK fucks up, we might want to check if it still works with a simple
// application
#ifdef DUMMY_DPDK
/*
...
int fake_io_launch(int argc, char *argv[], void (*nf)(void)) {
...
#elif defined(FAKE_DPDK_IO_XCHG) || defined(FAKE_DPDK_IO_ASQ_OFFLOAD_TX)
    printf("Using XCHG\n");
    mbuf_pool =
    ...
    rte_eal_mp_remote_launch(nf, NULL, CALL_MASTER);
#elif defined(FAKE_DPDK_IO_DPDK) || defined(FAKE_DPDK_IO_ASQ_OFFLOAD_TX)
    rte_eal_mp_remote_launch(nf, NULL, SKIP_MAIN);
#endif
