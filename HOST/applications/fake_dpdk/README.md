# ASNI & IO abstractions

This is a small documentation to explain how we abstracted IO (DPDK & XCHG) and ASNI behind a single API.
...
while (1){
    ...
    ASNI_IO_RX_BURST(port, queue, descriptors, BATCH_SIZE, rx_count, state);
    // You can then iterate over packets
    for (uint16_t n = 0; n < rx_count; n++) {
        // Setup metadata, it is handled by the API and not a concern for users
        ASNI_IO_SETUP_METADATA(descriptors[n], rx_count, metadata);
        // Retrieve pointer to payload
        uint8_t *data;
        ASNI_IO_GET_PAYLOAD_PTR(descriptors[n], data);
        ...
        if (decision){
            uint16_t size;
            ASNI_IO_GET_IMPLICIT_PAYLOAD_SIZE(descriptors[n], size);
            uint8_t *payload;
            ASNI_IO_GET_IMPLICIT_PAYLOAD_PTR(descriptors[n], payload);
            ASNI_IO_TX_ENQUEUE(state, descriptors[n], subdescriptors[sub_desc].size, metadata, payload);
        } else {
            // Free if possible
            ASNI_IO_FREE_IMPLICIT(descriptors[n]);
        }
        // Allows the API to understand that you don't need the packet anymore
        ASNI_IO_END_PROCESS(descriptors[n]);
    }
    // Send enqueued packets and creates the tx_count variable
    ASNI_IO_TX_BURST(out_port, out_queue, rx_count, tx_count, state);
