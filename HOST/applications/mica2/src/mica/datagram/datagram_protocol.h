...
template <class T> class RequestBatchReader {
public:
  ...
  bool is_valid() {
    ...
    auto ip = reinterpret_cast<const rte_ipv4_hdr *>(this->get_data() +
    ...
    if (rte_be_to_cpu_16(udp->dgram_len) != ip_length - sizeof(rte_ipv4_hdr)) {
  ...
  bool find_next() {
    ...
    if (reinterpret_cast<const char *>(h_) + sizeof(RequestHeader) +
            ::mica::util::roundup<8>(get_key_length()) +
            ::mica::util::roundup<8>(get_value_length()) - this->get_data() >
        total_length_)
      return false;
...
// buffer.
template <class T> class RequestBatchBuilder {
public:
  typedef ::mica::processor::Operation Operation;
  typedef ::mica::table::Result Result;
  ...
  explicit RequestBatchBuilder(T *buf)
      : buf_(buf), h_(reinterpret_cast<RequestHeader *>(
                       this->get_data() + sizeof(RequestBatchHeader))),
        remaining_space_(kMaxValueLength), count_(0) {}

  RequestBatchBuilder(const RequestBatchBuilder &o)
      : buf_(o.buf_), h_(o.h_), remaining_space_(o.remaining_space_),
        count_(o.count_) {}
  ...
  void set_src_ipv4_addr(uint32_t v) {
    auto ip = reinterpret_cast<rte_ipv4_hdr *>(this->get_data() +
                                               sizeof(rte_ether_hdr));
    ip->src_addr = v;
  }
