...
template <class StaticConfig> DatagramServer<StaticConfig>::~DatagramServer() {}
...
void DatagramServer<StaticConfig>::generate_server_info() {
  ...
  for (size_t i = 0; i < eids.size(); i++) {
    ...
    oss << '[' << eids[i] << ',' << ei.owner_lcore_id << ",\""
        << ::mica::network::NetworkAddress::str_mac_addr(
               network_->get_mac_addr())
        << "\",\""
        << ::mica::network::NetworkAddress::str_ipv4_addr(
               network_->get_ip_addr())
        << "\"," << ei.udp_port << ']';
#else
    oss << '[' << eids[i] << ',' << ei.owner_lcore_id << ",\""
...
void DatagramServer<StaticConfig>::directory_proc() {
  ...
  if (directory_refresh_interval_ == 0)
    return;
...
template <class StaticConfig> void DatagramServer<StaticConfig>::run() {
...
// Launch workers.
  ...
  for (size_t id = 1; id < lcore_count; id++) {
    workers[id] =
        std::thread(&DatagramServer<StaticConfig>::worker_proc, this, id);
  }
  worker_proc(0);
  ...
  for (uint16_t lcore_id = 1; lcore_id < lcore_count; lcore_id++) {
    if (!rte_lcore_is_enabled(static_cast<uint8_t>(lcore_id)))
      continue;
...
void DatagramServer<StaticConfig>::worker_proc(uint16_t lcore_id) {
  ...
  std::vector<EndpointId> my_eids;
  {
    ...
    for (size_t i = 0; i < eids.size(); i++) {
      ...
      if (ei.owner_lcore_id == lcore_id)
        my_eids.push_back(eids[i]);
    }
  }
  if (my_eids.size() == 0) {
    if (StaticConfig::kVerbose)
  ...
  while (!stopping_) {
    ...
    if (++next_index == rx_tx_states.size())
      next_index = 0;
...
// Clean up.
#ifndef USE_ENSO
  for (auto &rx_tx_state : rx_tx_states)
...
void DatagramServer<StaticConfig>::check_pending_tx_full(
  ...
  if (rx_tx_state.pending_tx.count != StaticConfig::kTXBurst)
    return;
...
void DatagramServer<StaticConfig>::check_pending_tx_min(
    RXTXState &rx_tx_state) {
  if (rx_tx_state.pending_tx.count != StaticConfig::kTXMinBurst)
    return;
...
void DatagramServer<StaticConfig>::check_pending_tx_timeout(
  ...
  if (age < stopwatch_.c_1_usec() * StaticConfig::kTXBurstTimeout)
    return;
...
#ifdef USE_ASNI
    network_->release_dpdk(rx_tx_state.pending_tx.bufs[i]);
#else
...
void DatagramServer<StaticConfig>::RequestAccessor::setup(
    RXTXState *rx_tx_state, enso::RxPipe::MessageBatch<enso::PktIterator> bufs,
    uint64_t now) {
  ...
  uint8_t *buffer = rx_tx_state_->tx_pipe->AllocateBuf(
      1536 /** ROUNDED_ETHER_MAX_LEN */ * ::mica::network::Enso::kTXBurst);
...
template <class StaticConfig>
uint64_t
DatagramServer<StaticConfig>::RequestAccessor::get_key_hash(size_t index) {
  assert(index >= next_index_to_retire_);
  assert(index < next_index_to_prepare_);
...
template <class StaticConfig>
const char *
DatagramServer<StaticConfig>::RequestAccessor::get_key(size_t index) {
  assert(index >= next_index_to_retire_);
  assert(index < next_index_to_prepare_);
...
template <class StaticConfig>
size_t
DatagramServer<StaticConfig>::RequestAccessor::get_key_length(size_t index) {
  assert(index >= next_index_to_retire_);
  assert(index < next_index_to_prepare_);
...
template <class StaticConfig>
const char *
DatagramServer<StaticConfig>::RequestAccessor::get_value(size_t index) {
  assert(index >= next_index_to_retire_);
  assert(index < next_index_to_prepare_);
...
template <class StaticConfig>
size_t
DatagramServer<StaticConfig>::RequestAccessor::get_value_length(size_t index) {
  assert(index >= next_index_to_retire_);
  assert(index < next_index_to_prepare_);
...
template <class StaticConfig>
char *
DatagramServer<StaticConfig>::RequestAccessor::get_out_value(size_t index) {
  assert(index == next_index_to_retire_);
  (void)index;
...
template <class StaticConfig>
uint32_t
DatagramServer<StaticConfig>::RequestAccessor::get_opaque(size_t index) {
  assert(index >= next_index_to_retire_);
  assert(index < next_index_to_prepare_);
...
#ifdef USE_ENSO
  pending_response_batch_.b =
      RequestBatchBuilder<uint8_t>(rx_tx_state_->pending_tx.current_tx_buffer);
#else
...
#ifdef USE_ASNI
  pending_response_batch_.b = RequestBatchBuilder<PacketBufferDPDK>(buf);
#else
...
#ifdef USE_ENSO
#elif defined(USE_ASNI)
  server_->network_->release_dpdk(pending_response_batch_.b.get_buffer());
#else
