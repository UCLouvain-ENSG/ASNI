...
template <class StaticConfig> DatagramClient<StaticConfig>::~DatagramClient() {
  if (stopping_ == false) {
    stopping_ = true;
...
void DatagramClient<StaticConfig>::discover_servers(size_t min_servers) {
  ...
  while (true) {
    ...
    if (server_list.size() >= min_servers)
      break;
...
void DatagramClient<StaticConfig>::probe_reachability(volatile bool &quit) {
  ...
  for (size_t idx = 0; idx < my_eids.size(); idx++) {
    ...
    // Allocate TX state
    uint8_t *buffer = tx_pipe->AllocateBuf(1536 /** ROUNDED_ETHER_MAX_LEN */ *
    ...
    for (uint16_t remote_eindex = 0; remote_eindex < s.endpoint_count;
      ...
      while (stopwatch_.diff_in_cycles(stopwatch_.now(), start) <
        ...
        if (stopwatch_.diff_in_cycles(stopwatch_.now(), last_probe_sent) >=
          ...
          printf("probing local endpoints for lcore %zu to reach remote "
                 "endpoint %" PRIu32 " on server %s\n",
                 lcore_id, s.endpoints[remote_eindex].eid,
                 s.server_name.c_str());
      ...
      if (valid_local_eindices.size() == 0) {
        ...
        printf("no local endpoint for lcore %zu could reach remote endpoint "
               "%" PRIu32 " on server %s\n",
    ...
    for (uint16_t partition_id = 0; partition_id < s.partition_count;
      ...
      for (uint16_t remote_eindex = 0; remote_eindex < s.endpoint_count;
        ...
        if (s.endpoints[remote_eindex].owner_lcore_id !=
          ...
          continue;
        if (rs.e[partition_id].remote_eid_index == remote_eindex)
          continue;
...
void DatagramClient<StaticConfig>::probe_send(RXTXState &rx_tx_state,
  ...
  RequestBatchBuilder<PacketBuffer> b(rx_tx_state.pending_tx.current_tx_buffer);
  ...
  uint8_t *buffer = rx_tx_state.tx_pipe->AllocateBuf(
      1536 /** ROUNDED_ETHER_MAX_LEN */ * StaticConfig::kTXBurst);
...
void DatagramClient<StaticConfig>::handle_response(ResponseHandler &rh) {
  ...
  for (uint16_t eindex = 0; eindex < thread_state.endpoint_count; eindex++) {
    ...
    while (true) {
      ...
      if (!StaticConfig::kSkipRX) {
        ...
        for (auto pkt : batch) {
          ...
          while (r.find_next()) {
            ...
            if (thread_state.rd_items[rd].epoch != epoch)
              continue;
      ...
      if (count < StaticConfig::kRXBurst)
        break;
    }
  }
#else
...
template <class StaticConfig>
uint16_t
DatagramClient<StaticConfig>::calc_partition_id(uint64_t key_hash,
...
void DatagramClient<StaticConfig>::make_new_pending_request_batch(
  ...
  if (StaticConfig::kVerbose)
    printf("make_new_pending_request_batch()\n");
  prb.b =
      RequestBatchBuilder<uint8_t>(rx_tx_state.pending_tx.current_tx_buffer);
}
#else
template <class StaticConfig>
void DatagramClient<StaticConfig>::make_new_pending_request_batch(
...
void DatagramClient<StaticConfig>::flush_pending_request_batch(
    RXTXState &rx_tx_state, uint32_t server_index, PendingRequestBatch &prb,
    uint64_t now) {
  ...
  if (prb.round_robin) {
    prb.remote_eid_index++;
    if (prb.remote_eid_index == s.endpoint_count)
      prb.remote_eid_index = 0;
  }
...
void DatagramClient<StaticConfig>::flush_pending_request_batch(
  ...
  prb.b.set_src_ipv4_addr(
      rte_cpu_to_be_32(network_->get_endpoint_info(rx_tx_state.eid).ipv4_addr));
  prb.b.set_dest_ipv4_addr(dst_ip_addr++);

  prb.b.set_src_udp_port(
      rte_cpu_to_be_16(network_->get_endpoint_info(rx_tx_state.eid).udp_port));

  // printf("network_->get_endpoint_info(rx_tx_state.eid).udp_port: %" PRIu16
  // "\n",
  //        network_->get_endpoint_info(rx_tx_state.eid).udp_port);
...
void DatagramClient<StaticConfig>::report_status(double time_diff) {
  ...
  printf("RESULT-RX-MOPS %7.3lf\n",
         static_cast<double>(total_operations_succeeded) / time_diff /
             1000000.);
  printf("RESULT-TX-MOPS %7.3lf\n",
         static_cast<double>(total_operations_done) / time_diff / 1000000.);
  printf("RESULT-SUCESS-RATE %6.2lf\n", success_rate * 100.);
  printf("RESULT-REJECT-RATE %6.2lf\n", reject_rate * 100.);
  printf("RESULT-TIMEOUT-RATE %6.2lf\n", timeout_rate * 100.);
