...
#include "mica/network/asni/asni.h"
...
// The network type.
  ...
  typedef ::mica::network::ASNI<> Network;
#else
...
#ifdef USE_ASNI
  static constexpr uint16_t kRXBurst = 384;
  static constexpr uint16_t kTXBurst = 384;
  static constexpr size_t kMaxParsedRequestCount = 2048;
#else
  static constexpr uint16_t kRXBurst = 32;
  static constexpr uint16_t kTXBurst = 32;
  static constexpr size_t kMaxParsedRequestCount = 512;
#endif
...
template <class StaticConfig = BasicDatagramServerConfig> class DatagramServer {
public:
...
#ifdef USE_ASNI
  typedef typename Network::PacketBufferDPDK PacketBufferDPDK;
#endif
  struct RXTXState {
    EndpointId eid;
...
#elif defined(USE_ASNI)
      std::array<PacketBufferDPDK *, StaticConfig::kTXBurst> bufs;
#else
...
#ifdef USE_ASNI
      RequestBatchBuilder<PacketBufferDPDK> b;
#else
