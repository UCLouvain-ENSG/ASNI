...
template <class StaticConfig>
typename LTable<StaticConfig>::Bucket *
LTable<StaticConfig>::get_extra_bucket(uint32_t extra_bucket_index) {
  assert(extra_bucket_index != 0);
  assert(extra_bucket_index < 1 + num_extra_buckets_);
  return extra_buckets_ +
         extra_bucket_index; // extra_buckets[1] is the actual start
...
size_t LTable<StaticConfig>::get_empty(Bucket *bucket,
  ...
  while (true) {
    ...
    if (!has_extra_bucket(current_bucket))
      break;
...
size_t LTable<StaticConfig>::find_item_index(
  ...
  // for (size_t i = 0; i < StaticConfig::kBucketSize; i++) {
  //   if (current_bucket->item_vec[i] != 0) {
  //     printf("item_vec[%lu] %lu\n", i, current_bucket->item_vec[i]);
  //     printf("get_tag(item_vec) %lu, tag %lu\n",
  //            get_tag(current_bucket->item_vec[i]), tag);
  //   }
  // }

  while (true) {
    // printf("StaticConfig::kBucketSize %d\n", StaticConfig::kBucketSize);
    for (size_t item_index = 0; item_index < StaticConfig::kBucketSize;
         item_index++) {
      uint64_t item_vec = current_bucket->item_vec[item_index];
      // printf("get_tag(item_vec) %lu, item_index %lu\n", get_tag(item_vec),
      //        item_index);
      // printf("tag %d\n", tag);
      if (get_tag(item_vec) != tag) {
        // printf("tag not match\n");
        // printf("get_tag(item_vec) %u, tag %u\n",
        //        (unsigned int)get_tag(item_vec), (unsigned int)tag);
        continue;
      }
...
template <class StaticConfig>
size_t
LTable<StaticConfig>::find_same_tag(const Bucket *bucket, uint16_t tag,
...
template <class StaticConfig> void LTable<StaticConfig>::cleanup_all() {
  if (std::is_base_of<::mica::pool::CircularLogTag,
                      typename Pool::Tag>::value) {
