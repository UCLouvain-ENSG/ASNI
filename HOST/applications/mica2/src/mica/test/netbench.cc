...
template <typename T> static uint64_t hash(const T *key, size_t key_length) {
...
int worker_proc(void *arg) {
  ...
  ResponseHandler rh;
  // retrieve ENV var MICA_NB_ITEMS
  const char *env_nb_items = std::getenv("MICA_NB_ITEMS");

  size_t num_items = atoi(env_nb_items);
  printf("num_items: %lu\n", num_items);
...
int main(int argc, const char *argv[]) {
  ...
  if (rte_eth_xstats_get_names(0, xstats_names, num_xstats) != num_xstats) {
    printf("Cannot get xstats\n");
  }
