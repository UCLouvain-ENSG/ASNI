...
CircularLog<StaticConfig>::CircularLog(const ::mica::util::Config &config,
  ...
  while (true) {
    ...
    if (!alloc_->map(
            alloc_id,
            reinterpret_cast<void *>(reinterpret_cast<char *>(data_) + size_),
            0, kWrapAroundSize)) {
      alloc_->unmap(data_);
      // TODO: Give up after some trials.
      continue;
...
template <class StaticConfig> CircularLog<StaticConfig>::~CircularLog() {
  if (!alloc_->unmap(data_))
    assert(false);
  if (!alloc_->unmap(data_ + size_))
    assert(false);
}

template <class StaticConfig>
typename CircularLog<StaticConfig>::Offset
CircularLog<StaticConfig>::allocate(size_t item_size) {
...
template <class StaticConfig> void CircularLog<StaticConfig>::reset() {
  head_ = tail_ = 0;
}

template <class StaticConfig> void CircularLog<StaticConfig>::lock() {
...
template <class StaticConfig> void CircularLog<StaticConfig>::unlock() {
  if (!StaticConfig::kConcurrent)
    return;
...
template <class StaticConfig> void CircularLog<StaticConfig>::pop_head() {
