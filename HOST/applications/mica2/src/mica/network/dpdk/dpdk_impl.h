...
template <class StaticConfig> DPDK<StaticConfig>::~DPDK() {
  if (started_)
    stop();
...
template <class StaticConfig> void DPDK<StaticConfig>::init_mempool() {
...
template <class StaticConfig> void DPDK<StaticConfig>::start() {
  assert(!started_);
...
template <class StaticConfig> void DPDK<StaticConfig>::stop() {
  assert(started_);
...
template <class StaticConfig>
typename DPDK<StaticConfig>::PacketBuffer *
DPDK<StaticConfig>::clone(PacketBuffer *buf) {
  assert(buf);
  ...
  if (rx_packets > 0) {
    if (false) {
      ...
      for (size_t idx = 0; idx < rx_packets; idx++) {
        std::cout << "\tPkt " << idx << " of size " << bufs[idx]->get_length()
                  << "B (packet claims "
                  << sizeof(rte_ether_hdr) +
                         rte_be_to_cpu_16(
                             ((rte_ipv4_hdr *)((char *)bufs[idx]->get_data() +
                                               sizeof(rte_ether_hdr)))
                                 ->total_length)
                  << "B)" << std::endl;
      }
