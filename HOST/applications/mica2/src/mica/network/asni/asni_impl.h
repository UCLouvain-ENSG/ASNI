#pragma once
#ifndef MICA_NETWORK_ASNI_IMPL_H_
#define MICA_NETWORK_ASNI_IMPL_H_

namespace mica {
namespace network {
template <class StaticConfig>
ASNI<StaticConfig>::ASNI(const ::mica::util::Config &config)
...
template <class StaticConfig> ASNI<StaticConfig>::~ASNI() {
  if (started_)
    stop();
...
template <class StaticConfig>
void ASNI<StaticConfig>::add_endpoint(uint16_t lcore_id, uint16_t port_id) {
...
template <class StaticConfig>
void ASNI<StaticConfig>::init_eal(uint64_t core_mask) {
  rte_argc_ = 0;
  ...
  if (ret < 0) {
    fprintf(stderr, "error: failed to initialize ASNI EAL (err=%s)\n",
            rte_strerror(ret));
    assert(false);
    return;
  }
...
template <class StaticConfig> void ASNI<StaticConfig>::init_mempool() {
  // Initialize mempool.  Note that mempool will not be freed because ASNI
  // does not support it.
  ...
  for (uint16_t numa_id = 0; numa_id < StaticConfig::kMaxNUMACount; numa_id++) {
    ...
    // adjusted in ASNI's .config file: CONFIG_RTE_MEMPOOL_CACHE_MAX_SIZE
...
template <class StaticConfig>
std::vector<typename ASNI<StaticConfig>::EndpointId>
ASNI<StaticConfig>::get_endpoints() const {
...
template <class StaticConfig>
const typename ASNI<StaticConfig>::EndpointInfo &
ASNI<StaticConfig>::get_endpoint_info(EndpointId eid) const {
  assert(eid < endpoint_count_);
  return endpoint_info_[eid];
}

template <class StaticConfig> void ASNI<StaticConfig>::start() {
  assert(!started_);
  ...
  // eth_conf.rxmode.max_lro_pkt_size = RTE_ETHER_MAX_LEN;
  ...
  // eth_conf.intr_conf.mode = RTE_FDIR_MODE_PERFECT;
  // eth_conf.intr_conf.pballoc = RTE_FDIR_PBALLOC_64K;
  // eth_conf.intr_conf.status = RTE_FDIR_NO_REPORT_STATUS;
  // eth_conf.intr_conf.mask.dst_port_mask = 0xffff;
  // eth_conf.intr_conf.drop_queue = 0;
  //
  ...
  for (uint16_t port_id = 0; port_id < ports_.size(); port_id++) {
    if (!ports_[port_id].valid)
      continue;
    if (ports_[port_id].next_available_queue_id == 0)
      continue;
    ...
    ret = rte_eth_promiscuous_enable(static_cast<uint8_t>(port_id));
    /* End of setting RX port in promiscuous mode. */
    if (ret < 0) {
      fprintf(stderr,
              "error: failed to set promis mode on port %" PRIu16 " (err=%s)\n",
              port_id, rte_strerror(ret));
      assert(false);
      return;
    }
    ...
    ret = rte_eth_dev_set_mtu(static_cast<uint8_t>(port_id), 9978);
    if (ret < 0) {
...
template <class StaticConfig> void ASNI<StaticConfig>::stop() {
  assert(started_);
...
template <class StaticConfig>
typename ASNI<StaticConfig>::PacketBufferDPDK *ASNI<StaticConfig>::allocate() {
  auto buf = reinterpret_cast<PacketBufferDPDK *>(
...
template <class StaticConfig>
typename ASNI<StaticConfig>::PacketBuffer *
ASNI<StaticConfig>::clone(PacketBuffer *buf) {
  assert(buf);
...
static inline void free_large_packet(struct metadata_tx_asni_mica *metadata) {
  rte_pktmbuf_free(metadata->mb);
  rte_mempool_put(metadata->metadata_pool, metadata);
}
template <class StaticConfig>
void ASNI<StaticConfig>::release(PacketBuffer *buf) {
  struct metadata_tx_asni_mica *md =
      (struct metadata_tx_asni_mica *)buf->opaque;
  md->refcnt--;
  if (md->refcnt <= 0) {
...
template <class StaticConfig>
void ASNI<StaticConfig>::release_dpdk(PacketBufferDPDK *buf) {
  rte_pktmbuf_free(buf);
}

template <class StaticConfig>
uint16_t ASNI<StaticConfig>::receive(EndpointId eid, PacketBuffer **bufs,
                                     uint16_t buf_count) {
  assert(eid < endpoint_count_);
  auto &ei = endpoint_info_[eid];

  static struct rte_mempool *metadata_pool;
#ifdef COLLECT_RX_STATS
  static double rx_burst_sizes;
  static double rx_burst_nbs;
  static double rx_desc;
  static double rx_nb_desc;
#endif
  if (unlikely(metadata_pool == NULL)) {
    metadata_pool = rte_mempool_create("metadata_pool", 8192,
                                       sizeof(struct metadata_tx_asni_mica), 0,
  ...
  struct rte_mbuf *tmp_bufs[32];
  uint16_t nb_rx = rte_eth_rx_burst(static_cast<uint8_t>(port_id), queue_id,
                                    tmp_bufs, buf_count / 48);
  int buf_idx = 0;
  for (uint16_t i = 0; i < nb_rx; i++) {
    struct metadata_tx_asni_mica *metadata;
    uint32_t offset_desc = 16;
    uint8_t *data = rte_pktmbuf_mtod(tmp_bufs[i], uint8_t *);
    uint8_t nb_desc = *data;
    ei.rx_packets += nb_desc;
#ifdef COLLECT_RX_STATS
    rx_desc += nb_desc;
    rx_nb_desc++;
#endif
    char *payload =
        (char *)(data + offset_desc + (nb_desc * sizeof(struct descriptor)));
    if (unlikely(nb_desc < 1 || nb_desc > 64)) {
      printf("received packet with wrong number of descriptors : %d\n",
             nb_desc);
    } else {
      ...
      desc->payload_ptr = payload;
      desc->opaque = (void *)metadata;
      bufs[buf_idx++] = reinterpret_cast<PacketBuffer *>(desc);
  ...
  if (buf_idx > 0) {
#ifdef COLLECT_RX_STATS
    rx_burst_sizes += buf_idx;
    rx_burst_nbs++;
    if (unlikely(rx_burst_nbs == 1000000)) {
      printf("average burst size : %f\n", rx_burst_sizes / rx_burst_nbs);
      printf("average desc per packet : %f\n", rx_desc / rx_nb_desc);
      rx_burst_sizes = 0;
      rx_burst_nbs = 0;
    }
#endif
...
template <class StaticConfig>
uint16_t ASNI<StaticConfig>::send(EndpointId eid, PacketBufferDPDK **bufs,
                                  uint16_t buf_count) {
  assert(eid < endpoint_count_);
  auto &ei = endpoint_info_[eid];
  ...
  for (uint16_t i = tx_packets; i < buf_count; i++)
    release_dpdk(bufs[i]);
