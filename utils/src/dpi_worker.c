...
printf_signature(struct doca_dpi_worker_ctx *dpi_ctx, uint32_t sig_id, uint32_t fid, bool blocked)
{
	...
	if (likely(result == DOCA_SUCCESS))
		...
		DOCA_LOG_ERR("Failed to get signatures: %s", doca_error_get_descr(result));
}
...
dpi_worker_lcores_run(int nb_queues, struct dpi_worker_attr attr)
{
	...
	RTE_LCORE_FOREACH_WORKER(lcore) {
		...
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Buf_inventory create failure, %s", doca_error_get_descr(result));
			dpi_worker_cleanup(ctx);
			return result;
		}
		result = doca_buf_inventory_start(ctx->buf_inventory);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Buf_inventory start failure, %s", doca_error_get_descr(result));
			dpi_worker_cleanup(ctx);
			return result;
		}
		...
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("DPDK bridge_mempool create failure, %s", doca_error_get_descr(result));
			dpi_worker_cleanup(ctx);
			return result;
		}
		result = doca_dpdk_mempool_dev_add(ctx->doca_dpdk_pool, attr.dpi_ctx->dev);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("DOCA DPDK mempool dev add failure, %s", doca_error_get_descr(result));
			dpi_worker_cleanup(ctx);
			return result;
		}
		result = doca_dpdk_mempool_start(ctx->doca_dpdk_pool);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("DOCA DPDK mempool start failure, %s", doca_error_get_descr(result));
			dpi_worker_cleanup(ctx);
			return result;
		}
